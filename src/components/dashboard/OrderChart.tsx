...
const aggregateOrdersByDay = (orders: Order[], startDate: Date, endDate: Date): { [key: string]: number } =>
    orders
        .filter(order => order.deliveryStatus.id !== 4 && new Date(order.createdAt) >= startDate && new Date(order.createdAt) <= endDate)
        .reduce((acc, curr) => {
...
const aggregateOrdersByMonth = (orders: Order[]): { [key: string]: number } =>
    orders
        .filter(order => order.deliveryStatus.id !== 4)
        .reduce((acc, curr) => {
            const month = format(startOfMonth(new Date(curr.createdAt)), 'yyyy-MM');
...
const OrderChart = (props: { orders?: Order[] }) => {
    ...
    const uniqueMonths = useMemo(() => {
        const months = orders ? orders.map(order => format(startOfMonth(new Date(order.createdAt)), 'yyyy-MM')) : [];
